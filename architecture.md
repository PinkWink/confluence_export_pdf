...
confluence_print/
  ...
  history.html              — Project history document
...
CLI Input (Page URL)
  ...
  → Fork
      ...
      └── Presentation: build_presentation_html() → HTML + PDF (16:9 slides)
```
...
|---|------------|-----------------------|----------------------------------------------------|
| 1 | Cover Page | `build_cover_page()`  | Title at upper 3/4, PinkLAB logo at bottom 1/4     |
| 2 | TOC Page   | `build_toc_page()`    | Auto-generated from h1–h4 headings with numbering  |
| 3 | Content    | `build_html()`        | Processed Confluence body with styled elements      |
...
Each slide includes a topbar (section name | page title | PinkLAB logo) and a footer (author | page number). JS keyboard/wheel navigation for slideshow mode.
...
| `process_images(soup)`                | ac:image → img or video tags (detects video ext.)  |
...
| `build_presentation_html(data, id, soup)` | Assemble slide HTML with JS navigation          |
...
| `present_{title}.html` | HTML     | Presentation: interactive slides with keyboard/wheel nav     |
